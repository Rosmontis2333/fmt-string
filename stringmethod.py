    ...
    def compute_mep(
        ...
        for tstep in tqdm(range(1, maxsteps + 1)):
            ...
            if tstep % traj_every == 0:
                ...
                self.error.append(np.sqrt(np.mean((string - old_string) ** 2)))
        ...
        for i in range(len(string_L)):
            string_L[i][0] = y_min + y_step*((string_L[i][0]-x_min)/x_step)
            string_L[i][1] = x_min + x_step*((string_L[i][1]-y_min)/y_step)
        string = np.asarray(string_L)
